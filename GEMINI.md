...
- **Automation & Fastlane Mandate:** **ALWAYS** use **Fastlane** for all repetitive automation tasks, including building, testing, and deployment. **NEVER** use `xcodebuild test` or `xcodebuild test-without-building` directly; these commands are restricted via safety policy to ensure pre-build integrity (assets, strings, etc.) is maintained.
