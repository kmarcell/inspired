...
3.  **Community Feed & Post Card** (Unified Discovery & Mixed Content - 3-Tier Fallback Strategy)
...
7.  **Cross-Platform PWA Parity Specification** (Detailed blueprint in [`docs/pwa_feature_parity_matrix.md`](file:///home/marcellkresz/developer/inspired/docs/pwa_feature_parity_matrix.md))
...
| `companyId` | `String?` | Optional parent Company brand ID for multi-location businesses. | No |
...
Represents a parent brand or business organization (e.g. "Triyoga UK" or "Zen Sanctuary Group") that owns one or multiple studio location branches. A single user profile can own and manage multiple Company brands.

**JSON Example:**
```json
{
  "id": "company_zen_sanctuary_01",
  "ownerId": "usr_yogi_456",
  "name": "Zen Sanctuary Group",
  "contactEmail": "contact@zensanctuary.co.uk",
  "website": "https://zensanctuary.co.uk",
  "description": "Boutique hot vinyasa and restorative yoga sanctuaries across the UK.",
  "createdAt": "2026-08-28T15:26:00Z"
}
```
...
| Field | Type | Description | PII? |
| :--- | :--- | :--- | :--- |
| `id` | `String` | Unique Company ID. | No |
| `name` | `String` | Parent business / brand name. | No |
| `ownerId` | `String` | User UID of verified brand owner. | No |
| `contactEmail` | `String` | Business contact email address. | No |
| `website` | `String?` | Optional official website URL. | No |
| `description` | `String` | Brand overview and philosophy. | No |
| `createdAt` | `Timestamp` | Record creation timestamp. | No |
...
3.  **Upfront Privacy Control Matrix**:
    - Allows new users to configure `isProfilePublic`, `avatarPrivacy`, and `showJoinedGroups` immediately during onboarding.
    - Defaults to privacy-first state (`isProfilePublic: false`, `avatarPrivacy: "groups-only"`, `showJoinedGroups: "members-only"`).
...
### 5.8 Responsive Navigation & User Profile Screen
**Goal:** Provide an intuitive navigation structure across mobile/desktop web and native iOS, anchoring profile management and logout into a dedicated User Profile Screen.

1.  **Responsive Layout Strategy**:
    - **Desktop Web (`≥ md` screens)**: Left-hand persistent navigation sidebar (`w-64 fixed left-0 top-0 bottom-0`) displaying Inspired branding, user handle status, navigation links (🧘‍♀️ **Community Feed**, 👥 **My Communities**), and Logout button.
    - **Mobile App & PWA (`< md` screens)**: Sticky top bar with clickable Avatar/Handle button, and a floating bottom tab bar (`fixed bottom-0`). Main view container enforces `pb-28` safe bottom padding so post cards are never covered by navigation.

2.  **User Profile Screen (`/profile`)**:
    - **Trigger**: Tapping the top-left User Avatar or Handle in the header bar navigates to the **User Profile Screen**.
    - **UI & Sections**:
      - **Header & Avatar**: Displays high-resolution Avatar preview with edit/upload option, Display Name, and uneditable handle (`name#XXXX`).
      - **Bio**: Short biography editor (max 280 characters).
      - **Joined Communities Summary**: Overview of communities joined by the user with quick navigation.
      - **Privacy Control Matrix**: Granular toggles for `isProfilePublic`, `avatarPrivacy` (`public` | `groups-only`), and `showJoinedGroups` (`public` | `groups-only` | `members-only`).
      - **Account Actions**: Prominent **Log Out** button and Google / Email account session details.
...
### 5.11 Company & Multi-Studio Management Suite
**Goal:** Allow studio owners, teachers, and multi-location businesses to create Company Brands and manage Studio Locations across multiple postcodes.
**Mockup (Management Overview):** `UI/Mockups/5.11_StudioManagementSuite.svg`
**Mockup (Create Studio Form):** `UI/Mockups/5.11_CreateStudioForm.svg`

**Navigation:**
- **Desktop Sidebar:** Added **"🏢 My Studios"** tab in main left navigation bar.
- **Mobile Access:** Action card in **Profile View (`ProfileView.tsx`)** labeled **"🏢 My Studios & Company"**.
- **Header Action:** `← Back to Feed` button for seamless mobile navigation.

**Key Requirements:**
1. **Multi-Brand Support:** A single user profile (`ownerId`) can own and manage multiple **Company Brands** (e.g., "Zen Sanctuary Group", "Flow Hot Yoga Ltd").
2. **Multi-Location Branches:** Each Company Brand can contain multiple physical **Studio Locations** with distinct postcode outward codes (`location_prefix` e.g., `W12`, `W6`, `SW3`, `NW1`).
3. **Independent Studio Support:** Independent studios can be created directly without creating a parent company.
4. **Email Input Validation & Clear Button:**
   - Strict email regex validation (`/^[^\s@]+@[^\s@]+\.[^\s@]+$/`) across login, studio creation, and claim forms.
   - Interactive **"✕" clear button** on email input fields when text is present.
5. **Create Studio Form:**
   - **Step 1 (Company Brand):** Select existing Company Brand, select "Create New Company Brand", or select "Independent Studio (No Company Brand)".
   - **Step 2 (Studio Details):** Studio Name, Postcode Outward Code (`location_prefix`), Full Address, and About/Bio.
   - Validation: Required field checks and postcode prefix formatting (`W12`, `W6`, `SW3`).
   - Default Status: User-created studios default to `isClaimed: false` (Pending Admin Verification) and create a pending claim document in `/studioClaims`.
6. **Unified Studio Management Modal (`⚙️ Manage`):**
   - Studio location tiles feature a single **`⚙️ Manage`** button opening the unified management drawer.
   - **Studio Bio / About Text Editing:** Editable for verified studio owners (`isClaimed: true`), locked for unverified pending studios (`isClaimed: false`).
   - **Company Brand Reassignment:** Independent studios can be reassigned to any of the owner's Company Brands (or returned to Independent status) inside the Manage Studio drawer.
7. **Studio Operating Status & Soft Closure System:**
   - Three-tier operating status: `active` (Open & Active 🟢), `temp_closed` (Temporarily Closed ⏸️), and `closed` (Permanently Closed 🔴).
   - **Temporary Closure:** Supports custom reopening note (`statusNote` e.g., *"Closed for summer refurbishment until Sept 15"*). Renders **`⏸️ Temporarily Closed`** badge in search and studio lists.
   - **Soft Closure (Permanent Deletion):** Closing a studio marks `isClosed: true` and `status: 'closed'` rather than hard deleting the Firestore document, ensuring existing subscribed members still see historical studio context marked **`🔴 Permanently Closed`**. Includes a 1-tap **`🔄 Reopen Studio`** action.
8. **Real-Time Integration:** Claimed/created studio profiles immediately appear in search results (`searchEntities`) and community feed filters.
9. **Website URL Update & Admin Re-Verification Policy:**
   - Changing the official website URL of a verified studio or company brand (`isClaimed: true`) automatically flags the profile for Admin Re-Verification.
   - Reverts claim status to unverified (`isClaimed: false`) and generates a pending re-verification claim in `/studioClaims` (`Website URL updated to [new_url] - Re-verification required.`) until approved by an Admin.
10. **Company Brand Deletion & Reassignment Policy:**
    - Deleting a Company Brand does **not** delete its studio locations.
    - All studio locations under a deleted brand automatically revert to **Independent Studios** (`companyId: null`), where they can be reassigned to another brand at any time.

### 5.12 Admin Claim Review, Shadow Profile Claiming & Notification Suite
**Goal:** Provide studio owners with an automated & document-verified studio claim flow for shadow profiles, complete with transactional email templates and admin verification tools.
**Mockups:** `UI/Mockups/5.11_CreateStudioForm.svg`, `UI/Mockups/5.12_AdminClaimsReviewPortal.svg`

**Key Requirements:**
1. **Unclaimed Shadow Profile Badge & Claim Trigger:** Unclaimed studio tiles in search results (`isClaimed: false`) display an interactive **"Claim Studio 🏢"** button triggering the claim flow (`ClaimStudioView.tsx`).
2. **Outreach & Claim Email Template (`claim_invitation.html`):** Sends automated invitation email containing a direct claim token link (`https://inspired.yoga/claim-studio?studioId=...&token=...`).
3. **Verification Submission:** Claimant submits user identity and proof of ownership document (utility bill, insurance cert, business license) saved to `/studioClaims/{claimId}`.
4. **Approval Automated Email (`claim_approval.html`):** On claim approval (`approveStudioClaim`), the system automatically sends confirmation email with a direct dashboard link (`isClaimed: true`).
5. **Rejection Email with Optional Reason (`claim_rejection.html`):** On rejection (`rejectStudioClaim`), admin can specify an optional reason embedded in the claimant rejection email.
